...
contract Airdrop is IAirdrop, Ownable {
    ...
    function startCampaign(bytes32 merkleRoot, address token, uint96 duration) external onlyOwner {
        uint96 expired;
        uint256 timestamp = block.timestamp;
        unchecked {
            expired = uint96(timestamp) + duration;
        }
        _airdropCampaign[timestamp] = AirdropCampaign(merkleRoot, token, expired);

        emit CampaignStarted(timestamp);
    }

    function redeem(address to, uint256 amount, uint256 campaignId, bytes32[] calldata proofs) external payable {
    ...
    function _beforeRedeem(address account, uint256 amount, uint256 campaignId, bytes32[] calldata proofs) internal {
        ...
        view
    {
        ...
        bool valid = proofs.verify(root, keccak256(abi.encodePacked(account, amount)));
